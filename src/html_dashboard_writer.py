...
(() => {
  ...
  function chart(id,title,data,series,axisLabels={}){const box=document.getElementById(id),w=Math.max(360,box.clientWidth||720),h=Math.max(300,box.clientHeight||340);if(!data.length){box.innerHTML=`<div class="chart-empty">${esc(title)}：当前筛选条件无数据</div>`;return}const hasRight=series.some(s=>(s.axis||0)===1),leftLabel=axisLabels.left||series.filter(s=>(s.axis||0)===0).map(s=>s.name).join(' / '),rightLabel=axisLabels.right||series.filter(s=>(s.axis||0)===1).map(s=>s.name).join(' / '),L=leftLabel?86:64,R=hasRight&&rightLabel?86:40,T=72,B=54,pw=w-L-R,ph=h-T-B,axisMax=[0,0];series.forEach(s=>data.forEach(r=>axisMax[s.axis||0]=Math.max(axisMax[s.axis||0],Number(r[s.key])||0)));axisMax[0]=axisMax[0]||1;axisMax[1]=axisMax[1]||1;const axisSummary=leftLabel+((hasRight&&rightLabel)?`；右轴：${rightLabel}`:''),midY=T+ph/2;let out=`<svg viewBox="0 0 ${w} ${h}" role="img" aria-label="${esc(title+'；左轴：'+axisSummary)}"><text x="16" y="27" font-size="16" font-weight="700" fill="#243e60">${esc(title)}</text>`;let lx=16;series.forEach((s,i)=>{out+=`<line x1="${lx}" y1="50" x2="${lx+18}" y2="50" stroke="${colors[i]}" stroke-width="3"/><text x="${lx+24}" y="54" font-size="11" fill="#475569">${esc(s.name)}</text>`;lx+=Math.max(92,48+s.name.length*12)});if(leftLabel)out+=`<text data-axis-label="left" x="18" y="${midY}" text-anchor="middle" transform="rotate(-90 18 ${midY})" font-size="11" font-weight="600" fill="#475569">${esc(leftLabel)}</text>`;if(hasRight&&rightLabel)out+=`<text data-axis-label="right" x="${w-18}" y="${midY}" text-anchor="middle" transform="rotate(90 ${w-18} ${midY})" font-size="11" font-weight="600" fill="#475569">${esc(rightLabel)}</text>`;for(let i=0;i<=4;i++){const y=T+ph*i/4;out+=`<line x1="${L}" y1="${y}" x2="${L+pw}" y2="${y}" stroke="#e9eef5"/><text x="${L-8}" y="${y+4}" text-anchor="end" font-size="10" fill="#64748b">${(axisMax[0]*(4-i)/4).toFixed(axisMax[0]<20?1:0)}</text>`;if(hasRight)out+=`<text x="${L+pw+8}" y="${y+4}" font-size="10" fill="#64748b">${(axisMax[1]*(4-i)/4).toFixed(axisMax[1]<20?1:0)}</text>`}const step=data.length>1?pw/(data.length-1):0,skip=Math.max(1,Math.ceil(data.length/8));data.forEach((r,i)=>{if(i%skip===0||i===data.length-1){const x=L+(data.length>1?i*step:pw/2);out+=`<text x="${x}" y="${T+ph+22}" text-anchor="end" transform="rotate(-38 ${x} ${T+ph+22})" font-size="10" fill="#64748b">${esc(r.date.slice(5))}</text>`}});series.forEach((s,si)=>{const points=data.map((r,i)=>{const x=L+(data.length>1?i*step:pw/2),v=Number(r[s.key])||0,y=T+ph-v/axisMax[s.axis||0]*ph;return{x,y,v,date:r.date}});out+=`<polyline fill="none" stroke="${colors[si]}" stroke-width="2.5" points="${points.map(p=>p.x+','+p.y).join(' ')}"/>`;points.forEach(p=>out+=`<circle cx="${p.x}" cy="${p.y}" r="${data.length<16?3.5:2}" fill="${colors[si]}"><title>${esc(p.date+' '+s.name+'：'+p.v.toLocaleString())}</title></circle>`)});box.innerHTML=out+'</svg>'}
  function render(){const data=filteredRows(),sum=names=>data.reduce((a,r)=>a+num(r,names),0),sales=sum(['销售额','Ordered Product Sales']),orders=sum(['总订单','Units Ordered','订单量']),sessions=sum(['Sessions']),pv=sum(['PV','Page Views']),adSpend=sum(['广告花费','Spend']),adSales=sum(['广告销售额','Sales']),inventory=sum(['总库存']);const acos=adSales>0?adSpend/adSales*100:0;const metrics=[['记录数',data.length,'筛选后数据行'],['销售额',sales.toFixed(2),'筛选区间累计'],['订单量',orders.toFixed(0),'筛选区间累计'],['Sessions',sessions.toFixed(0),'业务流量'],['PV',pv.toFixed(0),'页面浏览量'],['广告花费',adSpend.toFixed(2),'筛选区间累计'],['广告销售额',adSales.toFixed(2),'筛选区间累计'],['ACoS',acos.toFixed(2)+'%','广告花费 ÷ 广告销售额']];cards.innerHTML=metrics.map(x=>`<div class="metric"><span>${esc(x[0])}</span><strong>${esc(x[1])}</strong><small>${esc(x[2])}</small></div>`).join('');const g=grouped(data);chart('trafficChart','业务流量趋势',g,[{name:'Sessions',key:'sessions'},{name:'PV',key:'pv'}],{left:'流量（Sessions / PV）'});chart('salesChart','销量与销售额趋势',g,[{name:'订单量',key:'orders'},{name:'销售额',key:'sales',axis:1}],{left:'订单量（单）',right:'销售额（金额）'});chart('adsChart','广告花费与销售额趋势',g,[{name:'广告花费',key:'adSpend'},{name:'广告销售额',key:'adSales',axis:1}],{left:'广告花费（金额）',right:'广告销售额（金额）'});chart('inventoryChart','库存趋势',g,[{name:'FBA可售库存',key:'fba'},{name:'总库存',key:'inventory'}],{left:'库存数量（件）'});filterSummary.textContent=`${store.value||'全部店铺'} · ${market.value||'全部站点'} · ${search.value||'全部商品'} · ${startDate.value||'最早'} 至 ${endDate.value||'最新'}`;const cols=['日期','shop_name','marketplace','SKU','ASIN','产品名称','Sessions','PV','总订单','销售额','广告花费','广告销售额','FBA可售库存','总库存'];count.textContent=`显示 ${data.length} 条记录`;head.innerHTML='<tr>'+cols.map(c=>`<th>${esc(c)}</th>`).join('')+'</tr>';body.innerHTML=data.slice(0,2000).map(r=>'<tr>'+cols.map(c=>`<td>${esc(r[c])}</td>`).join('')+'</tr>').join('');const q=payload.quality||[];quality.innerHTML=q.map(r=>`<div class="quality-row"><b>${esc(r['类别']||'问题')}</b>：${esc(r['内容']||JSON.stringify(r))}</div>`).join('')||'<span class="muted">未发现数据质量问题</span>'}
